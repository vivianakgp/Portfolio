...
FORM.addEventListener("click", (e) => {
  if (e.target.id === "name") {
    if (
      SPAN_EMAIL.classList.contains("isActive") ||
      SPAN_MESSAGE.classList.contains("isActive")
    ) {
  ...
  if (e.target.id === "email") {
    if (
      SPAN_NAME.classList.contains("isActive") ||
      SPAN_MESSAGE.classList.contains("isActive")
    ) {
...
const downloadPdf = (path, fileName) => {
  const downloadIntance = document.createElement("a");
  downloadIntance.href = path;
  downloadIntance.taget = "_blank";
  downloadIntance.download = fileName;
  document.body.appendChild(downloadIntance);
  downloadIntance.click();
  document.removeChild(downloadIntance);
};
